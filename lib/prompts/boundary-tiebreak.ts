...
//
// Its second round had the tie-break cite the WINNER's own fault tile and
// pick it anyway (2212.567: "Cut B ... splitting a spoken line", winner B,
// a real rule-8 split applied), and call a card on an END tile "buried"
// (2114.267, rule 7 read backwards). So each fault tile names its rule
// (a_fault_rule, b_fault_rule), the tile must be able to carry that rule
// (faultRuleProblem, shared with the skeptic), and a winner whose own
// fault tile is set is refused: both cuts break a rule, or the winner's
// tile is cleared.
...
export const TiebreakSchema = z.object({
  ...
  winner: z.enum(["A", "B", "neither"]).describe("the cut that satisfies rules 2-4 and 7-8 better: the winner breaks no rule (its fault tile is null); neither only when BOTH break a rule that shows on a tile of their own images (both fault tiles cited)"),
...
/** Where a side's cited fault tile is wrong, or null: it must be a tile of that side's own images, name its rule, and be a tile that can carry that rule against that side's cut (faultRuleProblem). */
export function faultTileProblem(side: TiebreakSide, t: number | null, rule: FaultRule | null = null): string | null {
  const field = `${side.label.toLowerCase()}_fault_tile_t`;
  if (t === null) {
    if (rule !== null) return `${side.label.toLowerCase()}_fault_rule ${rule} names a rule cut ${side.label} breaks but ${field} is null; cite the tile of ${side.label}'s own images that shows it, or clear the rule`;
    return null;
  }
  const img = side.images.find((i) => i.tiles.some((x) => Math.abs(x - t) <= SEEN_TOLERANCE_S + 1e-9));
  if (!img) return `${field} ${t} is not a tile of cut ${side.label}'s own images (its tiles: ${side.images.map((i) => `${i.tiles[0]}..${i.tiles[i.tiles.length - 1]}`).join(", ")})`;
  if (rule === null) return `${field} ${t} is cited without ${side.label.toLowerCase()}_fault_rule: name the standing decision (2-8) that tile shows cut ${side.label} breaking`;
  return faultRuleProblem(rule, t, img, side.t, field);
}
...
export function buildBoundaryTiebreak(input: BoundaryTiebreakInput) {
  ...
  return {
    ...
    description: "Record what each candidate cut's images show, the tile where each breaks a rule and the rule it breaks, then which cut satisfies the payoff rule, or neither, with the image and tile that decide it.",
    system,
    user,
    ...
    check: (out: TiebreakVerdict) => {
      ...
      const [a, b] = sides;
      const aProblem = faultTileProblem(a, out.a_fault_tile_t, out.a_fault_rule);
      if (aProblem) return aProblem;
      const bProblem = faultTileProblem(b, out.b_fault_tile_t, out.b_fault_rule);
      ...
      // A winner with its own fault tile cited (2212.567: "Cut B ... splitting a spoken line", winner B) is no winner.
      if (out.winner === "A" && out.a_fault_tile_t !== null) return "winner A but a_fault_tile_t cites A's own rule break; if both cuts break a rule answer neither, else clear the winner's fault tile";
      if (out.winner === "B" && out.b_fault_tile_t !== null) return "winner B but b_fault_tile_t cites B's own rule break; if both cuts break a rule answer neither, else clear the winner's fault tile";
