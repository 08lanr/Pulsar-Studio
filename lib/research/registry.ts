...
export const METRICS: MetricEntry[] = [
  ...
  },
  {
    ...
    question_zh: "平台当前把这部剧集放在榜单的什么位置？",
    grain: "listing",
    source_key: "engine",
  ...
  },
  {
    ...
    question_zh: "在同平台有计数的剧集中，它处在什么位置？",
    grain: "listing",
    source_key: "engine",
  ...
  },
  {
    ...
    limitations_zh: ["不是势头：老爆款和编辑推荐都会得高分。", "并列时，上榜剧集排在未上榜之前。"],
    status_rule: "catalog",
  },
  {
  ...
  },
  {
    ...
    question_zh: "某题材在显著剧集中的占比是否高于整体样本？",
    grain: "trope",
    source_key: "engine",
  ...
  },
  {
    key: "fresh_share",
    group: "derived",
    name_en: "Share of new listings",
    name_zh: "新作占比",
    question_en: "Which story types are the platforms launching right now?",
    question_zh: "平台现在正在上新哪些题材？",
    grain: "trope",
    source_key: "engine",
    source_field: "platform new flag, newest list, release date (≤ 30 days), first seen by Studio",
    unit: "share of the fresh cohort (0–1)",
    denominator: "listings that are new by at least one observed reason, in the latest snapshot",
    window: "latest published day",
    evidence: "inferred",
    formula: "fresh listings carrying the trope ÷ fresh listings; lift = that share ÷ the trope's share of every listing (needs ≥ 10 fresh listings)",
    version: "1.0",
    limitations_en: ["Multi-label: shares add to more than 100%.", "New means the platform listed it recently or Studio first saw it; it is not a measure of audience demand.", "ReelShort exposes no release date; its new flag and New shelf stand in."],
    limitations_zh: ["多标签：占比之和大于 100%。", "“新”指平台近期上架或 Studio 首次发现，不代表观众需求。", "ReelShort 不公开上线日期，以其新作标记和 New 栏目代替。"],
    status_rule: "catalog",
  },
  {
    key: "fresh_growth",
    group: "derived",
    name_en: "Growth of new listings",
    name_zh: "新作增长",
    question_en: "Which of the new listings is picking up fastest, and which story types do they carry?",
    question_zh: "哪些新作增长最快，属于什么题材？",
    grain: "listing",
    source_key: "engine",
    source_field: "views_counter on the two most recent published days",
    unit: "views per day, and % over the interval",
    denominator: "actual elapsed days between the two observations",
    window: "between the two most recent published days",
    evidence: "observed",
    formula: "same as counter velocity; a story type's figure is the median % across its fresh listings that have a baseline",
    version: "1.0",
    limitations_en: ["Within one platform only; never compared across platforms.", "A listing first seen today has no baseline and shows none.", "Two published days are needed; until then nothing is shown."],
    limitations_zh: ["仅在同一平台内比较。", "今天首次发现的剧集没有基线，不显示增长。", "需要两个已发布的日期，之前不显示。"],
    status_rule: "history",
  },
  {
    key: "trope_views_added",
    group: "derived",
    name_en: "Views added to a story type",
    name_zh: "题材新增观看",
    question_en: "Are listings of this story type being watched more since the last collection?",
    question_zh: "自上次采集以来，这类题材的剧集观看量增加了多少？",
    grain: "trope",
    source_key: "engine",
    source_field: "views_counter on the two most recent published days, every listing carrying the trope",
    unit: "counter units added over the window, per platform",
    denominator: "the summed baseline counters of the listings with a comparable counter on both days (for the %)",
    window: "between the two most recent published days",
    evidence: "observed",
    formula: "Σ(end − start) over listings of the trope on ONE platform with a comparable counter; % = that sum ÷ Σ start; per day = Σ per-listing velocity",
    version: "1.0",
    limitations_en: ["Per platform; never a total across platforms (different counters, different audiences).", "Listings first seen today or with a changed counter field are excluded and counted as the gap between listings and listings with a baseline.", "Counts what the platforms show on their public pages; not revenue, not unique viewers, not a US audience."],
    limitations_zh: ["按平台分别计算；不跨平台合计（计数口径与受众不同）。", "今天首次发现或计数字段变化的剧集不计入，作为“有基线的剧集数”与“剧集数”之差呈现。", "统计的是平台公开页面显示的计数；不是收入、不是独立观众、也不代表美国受众。"],
    status_rule: "history",
  },
  {
    ...
    limitations_zh: ["仅限同一平台同一计数。", "首次发现的剧集没有基线（不虚构零值）。"],
    status_rule: "history",
  },
  {
    key: "paywall_episode",
  ...
  },
  {
    ...
    limitations_zh: ["是与显著剧集的相似度，不是成功概率。", "标签来自你的简介，简介太短就匹配不到。"],
    status_rule: "producer",
  },
  {
  ...
  },
  {
    ...
    question_zh: "竞品在为哪些剧集投放广告，用什么素材？",
    grain: "listing",
  ...
  },
  {
    ...
    question_zh: "某剧集或题材在美国的搜索兴趣是否上升？",
    grain: "trope",
  ...
  },
  {
    ...
    question_zh: "哪些剧集的 YouTube 视频播放在增长？",
    grain: "listing",
