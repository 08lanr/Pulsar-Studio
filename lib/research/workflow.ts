...
export function campaignWorkflow(c: PromoCampaignSummary, results: CreativeResult[]) {
  ...
  const handedOff = ["submitted", "launching", "live", "failed"].includes(c.status);
  ...
  return { step, number: WORKFLOW_STEPS.indexOf(step) + 1, waiting, hasResults,
    ...
    hint: c.status === "failed" ? "workflow.hint.failed" : c.status === "generating" ? "workflow.hint.generating" : waiting ? "workflow.hint.waiting" : `workflow.hint.${step}` };
}
