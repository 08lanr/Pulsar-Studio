...
// one authenticated GET /api/studio/stats through the Studio API transport
// (the fake in fixture mode; the live site in Supabase mode, or from fixture
// mode with the engineer's CRAZYDRAMAS_LIVE_READ=1), parsed with the
// whitelist in ./stats-types.ts and kept for five minutes so a page that is
// reloaded, or opened on two tabs, asks crazydramas once. Server-only; the
// sums the screens show are the pure functions in ./stats-summary.ts.
...
export type CdStatsRead =
  | { ok: true; report: CdStatsReport; read_at: string; mode: "fake" | "live" }
...
export async function readCrazydramasStats(opts: { fresh?: boolean; transport?: CrazydramasStudioTransport; now?: () => number } = {}): Promise<CdStatsRead> {
  ...
  const read = { ok: true as const, report: parsed.data, read_at: new Date(now()).toISOString(), mode };
  cache.set(mode, { at: now(), read });
  return read;
}
