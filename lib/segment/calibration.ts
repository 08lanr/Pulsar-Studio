 ...
 * bad override and 2 hand-offs per 20, 9 of 10 card boundaries on the first
 * frame after the card, at most $0.40 per boundary. A sample that starts at
 * index 21 or later is scored as held out.
 */
export function calibrationBar(score: Pick<Score, "n" | "applied_agree" | "handoffs" | "cards" | "rule_failures"> & { skeptic: Pick<Score["skeptic"], "bad_overrides"> }, indices: number[], costCents: number): BarLine[] {
  const n = score.n || 1;
  const heldOut = indices.length > 0 && indices[0] >= 21;
  const need = Math.ceil(((heldOut ? 14 : 15) / 20) * n);
  const per20 = (x: number) => (x * 20) / n;
  return [
    { line: `applied agreement ${score.applied_agree}/${n} (bar ${need}/${n}, ${heldOut ? "held-out 14/20" : "tuning 15/20"})`, pass: score.applied_agree >= need },
    ...
    { line: `hand-offs to a person ${score.handoffs} (bar at most 2 per 20; each must be a real fault when checked by eye)`, pass: per20(score.handoffs) <= 2 + 1e-9 },
    { line: `card boundaries ${score.cards.agree}/${score.cards.boundaries} on the first frame after the card (bar 9 of 10)`, pass: score.cards.boundaries === 0 || score.cards.agree >= Math.ceil(0.9 * score.cards.boundaries) },
