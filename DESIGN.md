...
## One title workspace and the portfolio catalog (9 September 2026, later)

Three domains stay apart in every label and hue: preparation (neutral), TikTok monetization (success green accent), advertising (brand blue accent); every chip carries text. My catalog defaults to the portfolio row (TikTok status, ad status, TikTok earnings, ad spend, ad CTR, open) with quick filters whose definitions are written on the page; the readiness and TikTok comparisons are sibling views, not competing homepages. Every title page shares one shell: identity, the two statuses, a five-section nav (Overview · TikTok revenue & audience · Ad campaigns · Preparation · Materials) and the route back. Overview is four summary cards and one next action; detail pages are for investigation. The terminology map in `docs/terminology.md` governs labels in both languages.
