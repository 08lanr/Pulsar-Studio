...
export default function NewPromoForm({ titles, initialTitleId, readOnly = false }: { titles: TitleChoice[]; initialTitleId?: string; readOnly?: boolean }) {
  ...
  async function submit(e: React.FormEvent) {
    e.preventDefault();
    if (!name.trim() || hypothesis.trim().length < 10 || audience.trim().length < 3 || !destination.trim()) return;
  ...
  return (
    <form className="promo-brief" onSubmit={submit}>
      <fieldset disabled={readOnly || busy} className="promo-brief-main card" style={{ margin: 0, minWidth: 0 }}>
        ...
        <div className="field"><label className="label" htmlFor="np-destination">{tt("promote.new.destination")}</label><input id="np-destination" aria-describedby="np-destination-hint" className="input" type="url" value={destination} placeholder="https://" required onChange={(e) => setDestination(e.target.value)} /><p className="hint" id="np-destination-hint">{tt("promote.new.destinationHint")}</p></div>
