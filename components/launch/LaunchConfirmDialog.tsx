...
export default function LaunchConfirmDialog({ name, plan, destination, destinationNote, optimizes, pixelNote, runsAs, startPaused, provider, mode, accountNames, cards, pageDesign, adLine, staff, note, onNoteChange, error, onClose, onConfirm }: {
  name: string; plan: LaunchPlan; destination: string; startPaused: boolean;
  /** TikTok with Studio clips or the linked account's posts: the account the ads run as, and that clips stay off its profile (planIdentityLine). */
  runsAs?: string;
  /**
    ...
    <div className="launch-dialog launch-confirm-dialog" ref={panel} tabIndex={-1} role="dialog" aria-modal="true" aria-labelledby="launch-confirm-title" aria-describedby="launch-confirm-summary">
      ...
      <div className="launch-confirm-body">
        <dl className="launch-confirm-facts">
          ...
          {runsAs && <div className="launch-confirm-fact-wide"><dt>{tt("ltc.factRunsAs")}</dt><dd data-testid="confirm-runs-as">{runsAs}</dd></div>}
          ...
          return <article className="launch-confirm-row" key={row.index}>
            ...
            <div className="launch-confirm-ads">{ads.map(({ item, platform }, index) => {
              const card = cards?.[`${item.kind}:${item.value}`] ?? {
                platform: provider === "tiktok" ? "tiktok" as const : item.kind === "instagram_post" ? "instagram" as const : "facebook" as const,
