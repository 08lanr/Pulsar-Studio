...
export default function MetaSetup({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  return <div className="launch-flow"><div className="page-head"><div>{embedded ? <h2>{tt("lv2.meta.title")}</h2> : <h1>{tt("lv2.meta.title")}</h1>}<p className="page-sub">{tt("lv2.meta.sub")}</p></div><button className="btn btn-outline" onClick={() => void refresh()}>{tt("lv2.refresh")}</button></div>
