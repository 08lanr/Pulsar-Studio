          ...
          <tbody>{run.campaigns.filter(c => titleFilter === "all" || campaignTitleIds(run, c).includes(titleFilter)).map(c => {
            ...
            return <Fragment key={c.id}>
              ...
              {expanded[c.id] && <tr className="lm-detail-row"><td colSpan={9}><div className="lm-detail" id={`campaign-details-${c.id}`}>
                ...
                {run.draft.provider === "meta" && run.draft.meta_settings.conversion_event && c.snapshot && <p className="hint lm-web-line" data-testid="meta-conversions">{tt("lmx.monitorLine", { event: tt(`lmx.event.${run.draft.meta_settings.conversion_event}`), pixel: run.draft.meta_settings.pixel_id ?? "", attribution: tt("lmx.attribution") })}: {c.snapshot.conversions == null ? tt("lmx.conversionsNone") : int(c.snapshot.conversions)}{c.snapshot.conversion_value_cents != null ? ` (${money(c.snapshot.conversion_value_cents)})` : ""}</p>}
