...
export default async function Overview({searchParams}:{searchParams:Query}) {
 ...
 return <div className="market-brief"><div className="page-head"><div><span className="page-kicker">PULSAR / {t(locale,'research.market.kicker')}</span><h2>{t(locale,'research.market.title')}</h2><p className="page-sub">{t(locale,'ux.overview.sub')}</p></div><a className="brief-company-link" href="/producer/company">{company?(locale==='en'?company.name_en||company.name_zh:company.name_zh):t(locale,'ux.company')}<small>{t(locale,'ux.edit')} →</small></a></div>
