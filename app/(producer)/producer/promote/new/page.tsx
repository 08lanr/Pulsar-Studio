...
export default async function NewPromotion({ searchParams }: { searchParams: { title?: string } }) {
  ...
  return <><nav className="studio-crumbs"><a href="/producer/promote">{t(locale, "ws.nav.launch")}</a><span>›</span><span>{t(locale, "promote.new.title")}</span></nav><div className="page-head"><div><span className="page-kicker">{t(locale, "promote.new.kicker")}</span><h1>{t(locale, "promote.new.title")}</h1><p className="page-sub">{t(locale, "promote.new.sub")}</p></div></div>{picked ? <NewPromoForm titles={titles} initialTitleId={picked.id} readOnly={isStaffPreview(session)} /> : titles.length === 0 ? <section className="promo-empty"><h2>{t(locale, "promote.new.noTitles")}</h2><p>{t(locale, "promote.new.noTitlesHint")}</p><a className="btn btn-primary" href="/producer/titles/new?from=promote">{t(locale, "v3.nav.newTitle")}</a></section> : <><h2 className="field-group-title">{t(locale, "promote.new.pickTitle")}</h2><p className="page-sub">{t(locale, "promote.new.pickHint")}</p><section className="promo-campaign-grid">{titles.map((title) => { const body = <><header><span className={`pill ${title.hasVideo ? "pill-success" : "pill-warning"}`}>{t(locale, title.hasVideo ? "promote.new.pickReady" : "promote.new.noVideo")}</span></header><h3 className="bilingual">{title.name}</h3><footer><span>{title.episodeCount} {t(locale, "promote.new.episodes")}</span></footer></>; return <a key={title.id} className="promo-campaign-card promo-pick-card" href={`/producer/promote/new?title=${title.id}`}>{body}{!title.hasVideo && <p>{t(locale, "launch.planNote")}</p>}</a>; })}<a className="promo-campaign-card promo-pick-new" href="/producer/titles/new?from=promote"><IconPlus /> {t(locale, "v3.nav.newTitle")}</a></section></>}</>;
}
