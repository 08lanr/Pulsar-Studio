...
export default async function ExploreTitles({ searchParams }: { searchParams: Search }) {
 ...
 return <><ExploreNav active="titles" /><MarketFilters/><div className="rs-meta"><strong>{t(locale,'research.results',{n:sorted.length})}</strong><span>{t(locale,'research.page',{page:current,pages})}</span>{filter.q&&<span>{t(locale,'ux.searchMatch',{q:filter.q})}</span>}</div>
