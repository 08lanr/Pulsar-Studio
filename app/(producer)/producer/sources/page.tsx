...
export default async function SourcesPage(){
 ...
 return <><div className="page-head"><div><span className="page-kicker">{t(locale,'research.market.kicker')}</span><h2>{t(locale,'research.sources.title')}</h2><p className="page-sub">{t(locale,'ux.scope')} · {t(locale,'research.coverage.note')}</p></div></div><h3>{t(locale,'ux.sources.connected')}</h3><div className="brief-source-cards">{used.map(card)}</div>
 <section className="brief-section"><h3>{t(locale,'ux.sources.metrics')}</h3>{METRICS.filter(m=>['catalog','producer'].includes(m.status_rule)).map(m=><details className="brief-trope" key={m.key}><summary>{locale==='zh'?m.name_zh:m.name_en} <EvidenceTag evidence={m.evidence} locale={locale}/></summary><p>{locale==='zh'?m.question_zh:m.question_en}</p><ul>{(locale==='zh'?m.limitations_zh:m.limitations_en).map(l=><li key={l}>{l}</li>)}</ul><a href={`/producer/sources/${m.key}`}>{t(locale,'ux.advancedData')} →</a></details>)}</section>
