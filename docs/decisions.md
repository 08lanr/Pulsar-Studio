...
`resolveMetaPixel` (`lib/meta/pixel.ts`) reads `act_<id>/adspixels` and is asked twice: at preview, per chosen account, so the approver signs a pixel Meta has confirmed the account may use; and in the driver before its first write, recorded on the run. A pixel the list does not carry, or carries as `is_unavailable`, is refused in a sentence naming Business settings — not sent and rejected later. A resume that finds a different pixel than the one the run already created ad sets against stops rather than building a second ad set against another pixel.

While Meta refuses that read for want of the permission (codes 10, 200, 272, 3 — the permission family, which is where an unpublished app sits) the configured id stands in, marked unverified and said so in plain words, exactly as TikTok's 40001 fallback does. When the list answers, it wins.
