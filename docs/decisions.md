...
## 2026-09-09 (latest) · TikTok launch inside Studio: connect, launch, read back
...
- **Accounts are Pulsar's Business Center, assigned manually.** The operator connection is Pulsar Grow's: `.tokens.json` on the server from the admin **Connect TikTok** flow (`/tiktok`, staff admin, the existing production developer app), sandbox tokens from `.env.local`. Staff assign an ad account plus its publishing identity to a producer company (`core.company_accounts` gains `identity_id`, `identity_type`, `assigned_by`); only a staff-assigned, connected row is a launch account — a producer-recorded "connected" row never routes Pulsar's token anywhere. Producers see the launch account on Company & accounts and can **request one from Pulsar** (`core.account_requests`) with a mock payment opt-in (brand, holder, last four; never a card number; Studio charges nothing). The exact Business Center process stays manual by decision.
