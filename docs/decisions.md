...
- **The Pixel and Conversions API deferral is lifted.** Ruobin asked for it on 2026-09-22 ("we also have a tiktok business center and pixel for you to connect to"). It lands in the pixel phase of this build: landing URLs that name the ad group and the creative, the pixel as a staff-assigned company account, website-conversion ad groups, and TikTok-attributed purchase columns on the monitor. Until that phase lands, the "deferred; ask Ruobin" sentence in `CLAUDE.md` stays as written and is corrected then.
...
Checks: `npm test` (456 unit tests, 21 new), `npm run typecheck`, `npx next lint`.
