...
5. The pixel (Website purchases, the default shape): `TIKTOK_PIXEL_CODE=DALLBMJC77U250DBQUR0` (crazydramas.com's pixel; blank means the same). In Business Center, the pixel must be shared with the launch ad account (Assets → Pixel → Linked accounts). The `/tiktok` card shows "Shared · pixel id …" when `/pixel/list/` resolves it on the default account (a read-only call); "isn't shared with ad account … in Business Center" means share it first, and a refusal naming the pixel permission means the Studio TikTok app itself lacks it: add the pixel permission to the app in the TikTok developer portal first, then reconnect on `/tiktok`. Reconnecting without it brings back the same token and the same refusal. On 2026-09-23 the stored authorization (scopes 1, 2, 4, 6) read `advertiser/info/` for 7661702138107822098 but was refused `/pixel/list/` (40001, "advertiser does not grant you /pixel/list/:GET permission"), so every Website purchases preview is refused until this is done; launch with Traffic meanwhile. The Events API access token is an Events Manager token for `/event/track/`, not a Marketing API token, and cannot stand in for it. Studio sends no events and needs no Events API token; crazydramas.com sends them.
