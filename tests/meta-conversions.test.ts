...
import { CRAZYDRAMAS_META_PIXEL_ID, META_ACTION_TYPES, META_ATTRIBUTION_LABEL, metaPixelId } from "@/lib/meta/pixel";
import { metaConversionIssues, metaObjectiveSettings } from "@/lib/launch/plan";
...
function salesSettings(over: Partial<MetaLaunchSettings> = {}): MetaLaunchSettings {
  return { countries: ["US"], placements: ["facebook"], objective: "OUTCOME_SALES", optimization_goal: "OFFSITE_CONVERSIONS",
    conversion_event: "INITIATE_CHECKOUT", pixel_id: CRAZYDRAMAS_META_PIXEL_ID, bid_strategy: "LOWEST_COST_WITHOUT_CAP",
...
test("a Sales campaign names the pixel, the event and the attribution window on every ad set", async () => {
  ...
  assert.deepEqual(adset.promoted_object, { pixel_id: CRAZYDRAMAS_META_PIXEL_ID, custom_event_type: "INITIATE_CHECKOUT" });
...
test("Meta refusing the pixel list for want of the permission still launches, marked unverified", async () => {
  ...
  assert.deepEqual(sent(transport, "adsets")[0].promoted_object, { pixel_id: CRAZYDRAMAS_META_PIXEL_ID, custom_event_type: "INITIATE_CHECKOUT" });
...
test("an action Meta reports that is not the approved event is never counted as one", async () => {
  ...
  const driver = createMetaDriver(transport);
  await driver.launch(ctx);
  assert.notEqual(META_ACTION_TYPES.PURCHASE, META_ACTION_TYPES.INITIATE_CHECKOUT);
...
test("the objective and the goal move together, and switching back clears the pixel", () => {
  ...
  assert.equal(sales.conversion_event, "INITIATE_CHECKOUT");
...
test("the plan refuses every conversions combination Meta would reject", () => {
  ...
  assert.deepEqual(codes(salesSettings({ pixel_id: null })), ["conversionPixel"]);
